...
# Setup: 100Hz sample rate, 64 sample window (standard power of 2)
sim = sensor_sim.IMUSim(100.0, "walking")
extractor = sensor_sim.FeatureExtractor(64, 100.0) 

print("--- Simulation: Walking (Target: ~2.0 Hz) ---")
...
# Get features
feats = np.array(extractor.compute_features())

print("Feature Vector Extracted from C++:")
print(f"  [0] Mean Z-Accel:      {feats[0]:.4f} (Should be ~9.81)")
print(f"  [1] Var  Z-Accel:      {feats[1]:.4f} (Should be high)")
print(f"  [2] Dominant Freq:     {feats[2]:.4f} Hz (TARGET: ~2.0)")
print(f"  [3] Spectral Energy:   {feats[3]:.4f}")

if 1.5 < feats[2] < 2.5:
    print("\n✅ SUCCESS: FFT correctly identified the walking frequency!")
else:
    print("\n❌ FAIL: Frequency detection missed.")